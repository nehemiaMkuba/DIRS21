    ...
    public class HelperRepository
    {
        public static (bool IsValid, string Msisdn, string ErrorMessage) NormalizeMsisdn(string incomingMsidn, bool throwException = false)
        {
            ...
            if (!isValid)
            {
                ...
                if (throwException)
                {
                    throw new GenericException($"Invalid phone number{exception}", "DIRS21005", HttpStatusCode.PreconditionFailed);
                }
        ...
        public static (string EmailAddress, bool IsValid) ValidateEmailAddress(string emailAddress, bool throwException = false)
        {
            ...
            {
                if (throwException) throw new GenericException($"The email address provided is invalid", "DIRS21006", HttpStatusCode.BadRequest);
        ...
        public static void ValidatedParameter(string parameter, string value, out string result, bool throwException = false)
        {
            ...
            if (result.Length < 1 && throwException)
                throw new GenericException($"{parameter} must be provided to complete this request", "DIRS21007", HttpStatusCode.Forbidden);
        }
        ...
        public static void EvaluatePinStrength(string pin)
        {
            ...
            if (!valid) throw new GenericException($"PIN must be 4 digits long", "DIRS21024", HttpStatusCode.PreconditionFailed);
        }
        ...
        public static void EvaluatePasswordStrength(string password)
        {
            ...
            if (password.Length < 6) throw new GenericException($"Password must be at least 6 characters long", "DIRS21025", HttpStatusCode.PreconditionFailed);
        }
